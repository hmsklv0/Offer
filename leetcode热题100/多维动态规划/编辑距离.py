...
class Solution:
    ...
    def minDistance2(self, word1: str, word2: str) -> int:
        ...
        # 1 dp[i][j] 代表以i - 1结尾的word1转换成以 j-1 结尾的word2的最少操作数
        dp = [[0 for _ in range(n + 1)] for _ in range(m + 1)]
        ...
        #     dp[i][j] = dp[i-1][j-1]
        # else:
        #     # word1 删除 word1[i-1] (补缺的角度, 缺word1[i-1])
        #     dp[i][j] = dp[i-1][j] + 1
        #     # word1 插入 word2[j-1] (补缺的角度, 缺word2[j-1])
        #     dp[i][j] = dp[i][j-1] + 1
        #     # 替换
        #     dp[i][j] = dp[i-1][j-1] + 1

        # 3 初始化,如何初始化?一位一位修改，行为一位位删除word1[i-1]，初始列为一位位补word2[j-1]
        for i in range(m + 1):
            dp[i][0] = i
        for j in range(n + 1):
            dp[0][j] = j
